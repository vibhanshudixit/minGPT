...
## Causal/Masked Self- Attention[class CausalSelfAttention](../LLM/minGPT.py#L25)
...
## Multi-layer Perceptron[class MLP](../LLM/minGPT.py#L69)
...
## Defining the block[class Block](../LLM/minGPT.py#L85)
...
## configuring the GPT model parameters[class GPTConfig](../LLM/minGPT.py#L100)
- block_size = 1024
- vocab_size = 50304 
- n_layer = 12
- n_head = 12
...
## Defining the GPT class[class GPT](../LLM/minGPT.py#L109)
...
Then we created a function for calculating the number of parameters in the model[get_num_params](../LLM/minGPT.py#L138). This function excludes the position embeddings as we only take non-embedding count. We can also exclude the token embeddings, but as it is weight tyed with the weights in the final layer, we include them.

Initializing the weights using the [_init_weights](../LLM/minGPT.py#L150). where the weights for Linear and Embedding modules are normalized

Then we define the the forward function[def forward(self, idx, targets=None)](../LLM/minGPT.py#L158), where we forward the GPT function itself.
...
Lastly we create a function in the case we want to reduce the block_size from the existing 1024 block_size of GPT-2[def crop_block_size](../LLM/minGPT.py#L184).
...
Now we define the function from which we can call thr pretrained model[def from_pretrained](../LLM/minGPT.py#L197)
...
We will then configure the optimizers[def configure_optimizers](../LLM/minGPT.py#L254)
...
Then we create a function to estimate the MFU(Model FLOPs Utilization)[def estimate_mfu](../LLM/minGPT.py#L280)
- This is used to calculate the how much GPU's theoretical power is the model using.

Then finally, we create the generate function[def gneerate](../LLM/minGPT.py#L297)
